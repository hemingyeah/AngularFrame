//资讯列表
app.controller('KnowledgeCtrl', function($scope, $sce, Dialog, KnowledgeService, manageService, RoleService, service) {
        $scope.dataService = KnowledgeService;
        ...
        var manage = manageService.constructor($scope, $scope.gridOptions, "KnowledgeEditCtrl", "/views/KnowledgeManage/KnowledgeEdit.html");
        $scope.gridOptions.event = $.extend(manage, {
            GroupPush: function(data) {
                ...
                Dialog.Show("/views/KnowledgeManage/KnowledgeGroupPush.html", "KnowledgeGroupPushCtrl", "lg", {
            ...
            PersonalPush: function(data) {
                ...
                Dialog.Show("/views/KnowledgeManage/KnowledgePersonalPush.html", "KnowledgePersonalPushCtrl", "lg", {
                    SeletedData: function() {
                        return seletedData;
                    },
                    row: function() {
    ...
    //资讯编辑
app.controller('KnowledgeEditCtrl', function($scope, $sce, $modalInstance, Dialog, DataOperate, KnowledgeService, data) {
        $scope.model = data;
        $scope.event = {
            Save: function() {
                ...
                // $scope.model.KnowledgeLevelName = DataOperate.GetListName($scope.model.FkKnowledgeLevelId, $scope.KnowledgeLevels, "Id", "DisplayName");
                if ($scope.model.Id) {
                    KnowledgeService.PutKnowledge($scope, $.extend($scope.model, {
                        Id: $scope.Id
                    }), function(data) {
                        $modalInstance.close(data);
                    });
                } else {
                    KnowledgeService.PostKnowledge($scope, $scope.model, function(data) {
                        $modalInstance.close(data);
                    });
                }
            },
            ...
            fileChange: function(evt) {
                ...
                Dialog.Show("/views/KnowledgeManage/ImgCrop.html", "ImgCropCtrl", "lg", {
                    File: function() {
        ...
        KnowledgeService.LoadInfo($scope, {
            ID: $scope.model.Id
        }, function(data) {
            if (data.Id) {
        ...
        //     Save: function() {
        //         $scope.model.Content = $scope.editor.latestHtml;
        //         var ImgList = $('.richtext .Knowledge');
        //         var ImgSrcList = [];
        //         angular.forEach(ImgList, function(obj, index) {
        //             ImgSrcList.push(obj.currentSrc)
        //         });
    ...
    //个人资讯
app.controller('KnowledgePersonalPushCtrl', function($scope, $modal, $modalInstance, $sce, Dialog, KnowledgeService, service, RoleService, manageService, SeletedData) {
        ...
        $scope.event = {
            ...
            Push: function() {
                ...
                $scope.SeletedData.map(function(obj, index) {
                    KnowledgeIds.push(obj.Id)
                    return KnowledgeIds;
                })
                KnowledgeService.PostPushKnowledge($scope, {
    ...
    //群组资讯
app.controller('KnowledgeGroupPushCtrl', function($scope, $modal, $modalInstance, service, KnowledgeService, Dialog, RoleService, manageService, SeletedData) {
        $scope.SeletedData = SeletedData;
        $scope.Page = {
            Index: 1,
            Count: 1
    ...
    //标签资讯
app.controller('TagKnowledgeCtrl', function($scope, KnowledgeService) {
    $scope.Page = {
        Index: 1,
        Count: 1
    }
    $scope.event = {
        LoadData: function() {
            KnowledgeService.GetKnowledgesByUserId($scope, {
        ...
        Delete: function(data) {
            KnowledgeService.DeleteInfo($scope, data, function(data) {
...
app.controller('KnowledgeViewCtrl', function($scope, preview) {
