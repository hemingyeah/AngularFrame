//基础通知列表
app.controller('NoticeNewreleaseCtrl', function($scope, $sce, $compile, Dialog, NoticeNewreleaseService, manageService, RoleService, service) {
    $scope.dataService = NoticeNewreleaseService;
    ...
    $scope.ShowDetails = function(data, code, eventObj) {
        ...
        Dialog.Show("/views/NoticeNewManage/NoticeNewContextManage.html", "NoticeNewContextManageCtrl", "lg", {
            data: function() {
                return data;
            },
            rationType: function() {
    ...
    var manage = manageService.constructor($scope, $scope.gridOptions, "NoticeNewreleaseEditCtrl", "/views/NoticeNewreleaseManage/NoticeNewreleaseEdit.html");
    $scope.gridOptions.event = $.extend(manage, {
        RevokeInfo: function(data) {
            NoticeNewreleaseService.PutNoticeReleaseRevoke($scope, data, function(data) {
...
//基础通知编辑
app.controller('NoticeNewreleaseEditCtrl', function($scope, $sce, $modalInstance, Dialog, DataOperate, NoticeNewreleaseService, data) {
    $scope.model = data;
    $scope.event = {
        Save: function() {
            $scope.model.Content = $scope.editor.latestHtml;
            var ImgList = $('.richtext .NoticeNewrelease');
            var ImgSrcList = [];
            ...
            if ($scope.model.Id) {
                NoticeNewreleaseService.PutNoticeNewrelease($scope, $.extend($scope.model, {
                    Id: $scope.Id
                }), function(data) {
                    $modalInstance.close(data);
                });
            } else {
                NoticeNewreleaseService.PostNoticeNewrelease($scope, $scope.model, function(data) {
                    $modalInstance.close(data);
                });
            }
        },
    ...
    NoticeNewreleaseService.LoadInfo($scope, {
        ID: $scope.model.Id
    }, function(data) {
        if (data.Id) {
...
//个人通知
app.controller('NoticeNewreleasePersonalPushCtrl', function($scope, $modal, $modalInstance, $sce, Dialog, NoticeNewreleaseService, service, RoleService, manageService, SeletedData) {
    ...
    $scope.event = {
        ...
        Push: function() {
            ...
            NoticeNewreleaseService.PostPushNoticeNewrelease($scope, {
                NoticeNewreleaseId: $scope.SeletedData[0].Id,
                Recievers: recievers,
                PushType: 0
            }, function(data) {
...
//群组通知
app.controller('NoticeNewreleaseGroupPushCtrl', function($scope, $modal, $modalInstance, service, NoticeNewreleaseService, Dialog, RoleService, manageService, SeletedData) {
    $scope.SeletedData = SeletedData;
    $scope.Page = {
        Index: 1,
        Count: 1
...
//标签通知
app.controller('NoticeNewreleaseTagCtrl', function($scope, NoticeNewreleaseService) {
    $scope.Page = {
        Index: 1,
        Count: 1
    }
    $scope.event = {
        LoadData: function() {
            NoticeNewreleaseService.GetNoticeNewreleasesByUserId($scope, {
        ...
        Delete: function(data) {
            NoticeNewreleaseService.DeleteInfo($scope, data, function(data) {
...
app.controller('NoticeNewreleaseViewCtrl', function($scope, preview) {
    $scope.preview = preview;
});
//新通知细节列表
app.controller('NoticeNewContextManageCtrl', function($scope, $sce, $modalInstance, $compile, Dialog, NoticeNewreleasebehaviorService, manageService, RoleService, service, data, rationType) {
    $scope.data = data;
    $scope.rationType = rationType;
    $scope.dataService = NoticeNewreleasebehaviorService;
    $scope.roleSerivce = RoleService;
    manageService.dataGridInit($scope);
    NoticeNewreleasebehaviorService.LoadData($scope, $scope.data, function(data) {
