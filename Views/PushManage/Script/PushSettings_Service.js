app
	.factory('PushSettingsService', function($http, service, DataOperate) {
		var ret = {
			LoadPageData: function($scope, data, fun) {
			    if ($scope.tree) {
			        DataOperate.LoadData(Route.AppPushSetting_GetAppList, {
			...
			Save: function ($scope, data, fun) {
			    if (data.DataState === state.Add) {
			        ...
			        DataOperate.Edit(Route.AppPushSetting_PutAppPushSetting, $scope.model, function(data) {
			            if (fun) fun(data);
			        });
			    }
			},
